...
To illustrate the package, we use a go-to dataset in **R**. The `mtcars` dataset:
...
```{r}
library("igraph")
grph <- graph_from_adjacency_matrix(as.matrix(car_dist),
                                    mode = "undirected", weighted = TRUE,
                                    diag = FALSE)
tmp <- function(x) (x - min(x))/max(x - min(x))
E(grph)$width <-5*tmp(E(grph)$weight)
